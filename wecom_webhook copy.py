...
class WeComWebhook:
    ...
    def __init__(self, webhook_url: str, enabled: bool = True, log_callback=None, config: dict = None):
        ...
        Args:
            ...
            enabled: 是否启用推送（已废弃，使用群组配置）
            log_callback: 日志回调函数
            config: 配置字典（用于读取群组配置和频率限制）
        """
        ...
        # 频率限制相关
        self.rate_limit = self.config.get('wecom_webhook', {}).get('rate_limit', 60)
    ...
    def _get_text_suffix(self) -> str:
        """获取文字推送固定后缀"""
        webhook_config = self.config.get('wecom_webhook', {})
        return webhook_config.get('text_suffix', '【作者微信：MK0914666】')
    ...
    def _create_pdf_with_pil(self, title: str, content: str, author_name: str,
        ...
        try:
            ...
            # ============ 处理内容 ============
            if enable_noise:
                marked_content = self._process_content_with_noise(content, noise_text, min_noise_chars=20)
    ...
    def _process_content_with_noise(self, content: str, noise_text: str, min_noise_chars: int = 10):
        """
        ...
        if len(normal_indices) > min_noise_chars:
            noise_char_count = random.randint(min_noise_chars, min(min_noise_chars + 10, len(normal_indices)))

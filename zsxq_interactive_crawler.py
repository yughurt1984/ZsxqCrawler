...
class ZSXQInteractiveCrawler:
    ...
    def html_wrap_content(self, html_content: str, width: int = 25) -> str:
        """
        对 HTML 中的文本内容进行换行处理，避免 PDF 生成时文本溢出
        
        Args:
            html_content: 原始 HTML 内容
            width: 每行最大字符数（中文建议 20-30）
        
        Returns:
            处理后的 HTML 内容
        """
        try:
            # 使用正则表达式分割 HTML 标签和纯文本
    ...
    def convert_url_to_pdf(self, url: str, output_dir: str, title: Optional[str] = None, watermark_text: str = "六便士出品") -> Optional[str]:
        ...
        try:
            ...
            # ✅ 获取网页HTML（使用Edge浏览器headers）
            self.log(f"   🔍 获取网页HTML内容...")
            
            # ✅ 使用通用浏览器headers，而不是知识星球API headers
            parsed_url = urlparse(url)
            domain = parsed_url.netloc
            
            # ✅ 通用Edge浏览器headers（适用于外部网页）
            headers = {
            ...
            if "zsxq.com" in domain or "wx.zsxq.com" in domain:
                ...
                headers["Referer"] = f"https://wx.zsxq.com/"
            ...
            if response.status_code != 200:
                self.log(f"   ❌ 获取网页失败: HTTP {response.status_code}")
                return None
        
            html_content = response.text
            original_html_len = len(html_content)
            self.log(f"   📊 原始HTML长度: {original_html_len} 字符")
            
            # ✅ 预处理HTML：调整图片尺寸适应页面
            self.log(f"   🔧 预处理HTML：调整图片尺寸...")
            ...
            html_content = re.sub(r'<div[^>]*class="[^"]*milkdown-preview[^"]*"[^>]*>.*?</div>', '', html_content, flags=re.DOTALL)
            
            # 方法2：如果方法1没有匹配到，尝试单引号版本
            html_content = re.sub(r"<div[^>]*class='[^']*milkdown-preview[^']*'[^>]*>.*?</div>", '', html_content, flags=re.DOTALL)
            
            # 方法3：移除所有包含milkdown-preview的标签
            html_content = re.sub(r'<div[^>]*milkdown-preview[^>]*>.*?</div>', '', html_content, flags=re.DOTALL)
            
            cleaned_len = len(html_content)
            self.log(f"   ✅ 清理完成，长度: {cleaned_len} 字符（减少: {original_len - cleaned_len}）")
            
            # ✅ 使用xhtml2pdf从HTML转换PDF
            self.log(f"   🔍 开始使用xhtml2pdf转换PDF...")
            try:
                # ✅ 清理HTML结构，确保没有重复的xml声明和DOCTYPE
                # 移除已有的xml声明和DOCTYPE（避免重复）
                ...
                # ✅ 对 HTML 文本内容进行换行处理，避免 PDF 文本溢出
                self.log(f"   🔧 开始文本换行处理...")
                html_content = self.html_wrap_content(html_content, width=46)
                self.log(f"   ✅ 文本换行处理完成")
                
                 # ✅ 清理HTML中的多余空白和标签，优化段落间距
                self.log(f"   🔧 清理HTML结构...")
                
                # 1. 移除空的 <p> 标签（这是导致额外间距的主要原因）
                ...
                # 3. 移除图片后的空段落
                html_content = re.sub(r'<img[^>]+>\s*<p>\s*</p>', '</p>', html_content, flags=re.IGNORECASE)
                
                # 4. 合并连续的 <p> 标签，将 </p><p> 替换为单个 <br>
                html_content = re.sub(r'</p>\s*<p>', '<br>', html_content)
                
                # 5. 移除多余的连续 <br> 标签（保留最多1个）
                html_content = re.sub(r'(<br>\s*){2,}', '<br><br>', html_content)
                
                # 6. 移除 <p> 标签（如果内容已经在 <br> 分隔下）
                html_content = re.sub(r'<p([^>]*)>(.*?)</p>', r'\2', html_content, flags=re.DOTALL)
                
                # 7. 移除空的 div 标签
                html_content = re.sub(r'<div[^>]*>\s*</div>', '', html_content)
                
                # 8. 清理连续的空行（多个 <br> 合并为一个）
            ...
            if os.path.exists(pdf_path):
                file_size = os.path.getsize(pdf_path)
                self.log(f"   📊 PDF文件大小: {self.format_file_size(file_size)}")
                
                if file_size > 10 * 1024:
                    self.log(f"   ✅ PDF转换成功: {pdf_path}")
                    return pdf_path
                else:
                    self.log(f"   ⚠️ 警告：PDF文件过小（{self.format_file_size(file_size)}）")
                    return pdf_path
            else:
                self.log(f"   ❌ PDF文件未生成")
                return None

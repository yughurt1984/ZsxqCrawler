...
class IncrementalSyncManager:
    ...
    def _get_ssh_env(self) -> dict:
        """获取 SSH 环境变量，确保使用 cwRsync 的 Cygwin SSH"""
        env = os.environ.copy()
        import shutil
        rsync_path = shutil.which("rsync", path=env.get('PATH', ''))
        if rsync_path:
            cwrsync_bin = str(Path(rsync_path).parent)
    ...
    def execute_remote_sql(self, sql_statements: List[str], table_name: str, 
        ...
        for i in range(0, total_statements, batch_size):
            ...
            # 构建 SSH 命令
            ssh_cmd = ["ssh"]
            ssh_cmd.extend(self._build_ssh_args())
            ssh_cmd.extend([

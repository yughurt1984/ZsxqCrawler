...
class FileSyncManager:
    ...
    def build_rsync_command(self, group_id: str, directory: str, dry_run: bool = False) -> str:
        ...
        # SSH 选项 - 使用 Cygwin SSH（Windows SSH 与 Cygwin rsync 管道不兼容）
        # 密钥放在 cwRsync home 目录下，避免 NTFS 权限问题
        ssh_parts = [f"ssh -p {self.server_port}"]
        ssh_parts.append("-o StrictHostKeyChecking=no")
        ssh_parts.append("-o UserKnownHostsFile=/dev/null")
        ssh_parts.append("-o IdentitiesOnly=yes")
        ssh_parts.append("-i /home/arron/.ssh/id_rsa")
    ...
    def sync_directory(self, group_id: str, directory: str, dry_run: bool = False) -> bool:
        ...
        try:
            # 确保 Cygwin SSH 优先于 Windows SSH
            env = os.environ.copy()
            import shutil
            rsync_real_path = shutil.which(self.rsync_path, path=env.get('PATH', ''))
            if rsync_real_path:
                cwrsync_bin = str(Path(rsync_real_path).parent)
                env['PATH'] = cwrsync_bin + ";" + env['PATH']
            env['HOME'] = '/home/arron'
            env['CYGWIN'] = 'nontsec'
